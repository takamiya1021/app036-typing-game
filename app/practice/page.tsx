...
export default function PracticePage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-slate-900 flex items-center justify-center p-4 md:p-8">
      <div className="bg-white/5 backdrop-blur-lg rounded-2xl shadow-2xl p-4 md:p-8 max-w-6xl w-full">
        {/* ヘッダー */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 md:mb-8 gap-4">
          <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
            <Link
              href="/"
              className="px-3 py-2 md:px-4 md:py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors backdrop-blur-sm text-sm md:text-base"
              title="ホームへ戻る"
            >
              ← ホーム
            </Link>
            <h1 className="text-2xl md:text-4xl font-bold text-white">タイピング</h1>
          </div>
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => setIsApiKeyModalOpen(true)}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors backdrop-blur-sm flex items-center justify-center gap-2 text-sm md:text-base"
              title="APIキー設定"
            >
              🔑 AI設定
            </button>
